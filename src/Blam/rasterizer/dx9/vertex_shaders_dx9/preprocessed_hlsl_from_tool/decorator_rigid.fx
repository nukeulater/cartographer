...
#define COLOR		 c[207]
#define ACCURACY	 c[203].w
#define LIGHT_VECTOR c[203]


{
    float4 temp = (float4)(RIGID_MATRIX_X.wwww );
    	r11.x=temp.x;
}
{
    float4 temp = (float4)(RIGID_MATRIX_Y.wwww );
    	r11.y=temp.y;
}
{
    float4 temp = (float4)(RIGID_MATRIX_Z.wwww );
    	r11.z=temp.z;
}
{
    float4 temp = (float4)(r11.xxxx + -c[7] .xxxx );
    	r1.x=temp.x;
}
{
    float4 temp = (float4)(r11.yyyy + -c[7] .yyyy );
    	r1.y=temp.y;
}
{
    float4 temp = (float4)(r11.zzzz + -c[7] .zzzz );
    	r1.z=temp.z;
}
	r1 = dot(r1.xyz, r1.xyz);
 {
	...
	r1 = va_position * SCALE ;
 {
	float temptemp;
	temptemp = dot(r1.xyz, RIGID_MATRIX_X.xyz);
...
{
	float temptemp;
	temptemp = dot(r1.xyz, RIGID_MATRIX_Y.xyz);
...
{
	float temptemp;
	temptemp = dot(r1.xyz, RIGID_MATRIX_Z.xyz);
 	temptemp = temptemp + RIGID_MATRIX_Z.wwww ;
 {
	...
	r3 = va_normal ;
 {
    float4 temp = (float4)(dot(RIGID_MATRIX_X.xyz, r3.xyz));
    	r2.x=temp.x;
}
{
    float4 temp = (float4)(dot(RIGID_MATRIX_Y.xyz, r3.xyz));
    	r2.y=temp.y;
}
{
    float4 temp = (float4)(dot(RIGID_MATRIX_Z.xyz, r3.xyz));
    	r2.z=temp.z;
}


{
    float4 temp = (float4)(dot(r2.xyz, r2.xyz));
    	r2.w=temp.w;
}
{
    float4 temp = (float4)(rsqrt(r2.wwww ));
    	r2.w=temp.w;
}
{
    float4 temp = (float4)(r2.xyzz * r2.wwww );
    	r2.x=temp.x;
    	r2.y=temp.y;
    	r2.z=temp.z;
}
...
{
    float4 temp = (float4)(ACCURACY );
    	r1.w=temp.w;
}
{
    float4 temp = (float4)((-COLOR * r1.wwww ) + COLOR );
    	r1.x=temp.x;
    	r1.y=temp.y;
    	r1.z=temp.z;
}
{
    float4 temp = (float4)(COLOR + -r1.xyzz );
    	r3.x=temp.x;
    	r3.y=temp.y;
    	r3.z=temp.z;
}
...
{
    float4 temp = (float4)(dot(-LIGHT_VECTOR.xyz, r2.xyz));
    	r2.w=temp.w;
}
{
    float4 temp = (float4)(max(c[15].yyyy  , r2.wwww ));
    	r2.w=temp.w;
}
{
    float4 temp = (float4)((r3.xyzz * r2.wwww ) + r1.xyzz );
    	oD0.x=temp.x;
    	oD0.y=temp.y;
    	oD0.z=temp.z;
}
