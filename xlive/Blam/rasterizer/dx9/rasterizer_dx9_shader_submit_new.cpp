...
static bool g_spherical_light_error_displayed = false;
#endif

static bool g_use_secondary_gradient_index = false;
static bool g_use_gel_map_bitmap = true;
	...
	uint32* res_y) const
{
	...
	*res_y = 1;
	...
	case _shader_pass_texture_source_extern_light_gel:
		...
		if (light_type_is_spherical(current_light_type_get()) || light_def == NULL)
		{
#ifdef ERRORS_ENABLED
			if (!g_spherical_light_error_displayed)
			{
				...
				error(_error_delayed, string);
				g_spherical_light_error_displayed = true;
			}
#endif
			rasterizer_dx9_set_texture(stage, _bitmap_type_2d, bitmap_usage_default, NONE, 0, 0.f);
		}
		else if (light_def->flags.test(_light_definition_texture_camera_gel))
		{
			stage_texture(stage, _shader_pass_texture_source_extern_global_texture_camera, res_x, res_y);
		}
		else if (light_def->shader.index == NONE || rasterizer_rendering_light_get())
		{
