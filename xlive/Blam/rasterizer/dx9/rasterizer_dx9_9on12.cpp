...
HRESULT rasterizer_dx9_create_through_d3d9on12(IDirect3D9Ex** d3d, bool use_warp)
{
	...
	if (h_d3d12 == NULL
		...
		|| h_d3d9 == NULL)
	{
		error(_error_log, "Modules: d3d12, dxgi or d3d9 not available!");
		return E_FAIL;
	}
	...
	{
		error(_error_log, "Unable to resolve d3d12, dxgi or d3d9 function pointers!");
		return E_FAIL;
	}
	...
	if (FAILED(hr))
	{
		error(_error_log, "Failed to create DXGIFactory.");
		return E_FAIL;
	}
	...
	if (use_warp)
	{
		...
		{
			error(_error_log, "Failed to get WARP adapter.");
			return E_FAIL;
		}
	}
	else
	{
		...
		{
			error(_error_log, "Failed to get adapter.");
			return E_FAIL;
		}
	}
	...
	if (FAILED(hr))
	{
		error(_error_log, "Failed to create D3D12 device.");
		return E_FAIL;
	}
	...
	*d3d = (IDirect3D9Ex*)pfn_Direct3DCreate9On12(D3D_SDK_VERSION, &Args, 1);
	...
	if (FAILED(hr))
	{
		error(_error_log, "Failed to initialize D3D9On12.");
		return E_FAIL;
	}
