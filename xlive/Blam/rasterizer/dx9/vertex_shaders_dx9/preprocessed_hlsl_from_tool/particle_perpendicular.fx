...
{
    float4 temp = (float4)(dot(V_DIRECTION.xyz, c[168].yyxx.xyz));
    	r4.x=temp.x;
}
{
    float4 temp = (float4)(r4.xxxx * r4.xxxx );
    	r4.x=temp.x;
}

{
    float4 temp = (float4)(dot(V_DIRECTION.xyz, R_DIRECTION.xyz));
    	r4.y=temp.y;
}
{
    float4 temp = (float4)(r4.yyyy * c[168].zzzz );
    	r4.y=temp.y;
}

{
	float4 wookiewookie;
	hsge(wookiewookie, r4.x , r4.y );
{
    float4 temp = (float4)(wookiewookie);
    	r4.z=temp.z;
}
}
	R_SECOND_DIRECTION = c[168].xyyy + -c[168].yyxx ;
 	R_SECOND_DIRECTION = (r4.zzzz * R_SECOND_DIRECTION ) + c[168].yyxx ;
 

{
    float4 temp = (float4)(V_DIRECTION.yzxx * R_SECOND_DIRECTION.zxyy );
    	R_BASIS_FORWARD.x=temp.x;
    	R_BASIS_FORWARD.y=temp.y;
    	R_BASIS_FORWARD.z=temp.z;
}
{
    float4 temp = (float4)((-R_SECOND_DIRECTION.yzxx * V_DIRECTION.zxyy ) + R_BASIS_FORWARD );
    	R_BASIS_FORWARD.x=temp.x;
    	R_BASIS_FORWARD.y=temp.y;
    	R_BASIS_FORWARD.z=temp.z;
}
{
    float4 temp = (float4)(dot(R_BASIS_FORWARD.xyz, R_BASIS_FORWARD.xyz));
...
{
    float4 temp = (float4)(R_BASIS_FORWARD.xyzz * R_BASIS_FORWARD.wwww );
    	R_BASIS_FORWARD.x=temp.x;
    	R_BASIS_FORWARD.y=temp.y;
    	R_BASIS_FORWARD.z=temp.z;
}
{
    float4 temp = (float4)(V_DIRECTION.yzxx * R_BASIS_FORWARD.zxyy );
    	R_BASIS_LEFT.x=temp.x;
    	R_BASIS_LEFT.y=temp.y;
    	R_BASIS_LEFT.z=temp.z;
}
{
    float4 temp = (float4)((-R_BASIS_FORWARD.yzxx * V_DIRECTION.zxyy ) + R_BASIS_LEFT );
    	R_BASIS_LEFT.x=temp.x;
    	R_BASIS_LEFT.y=temp.y;
    	R_BASIS_LEFT.z=temp.z;
}
{
    float4 temp = (float4)(dot(R_BASIS_LEFT.xyz, R_BASIS_LEFT.xyz));
...
{
    float4 temp = (float4)(dot(R_DIRECTION.xyz, R_DIRECTION.xyz));
    	R_DIRECTION.w=temp.w;
}
{
    float4 temp = (float4)(rsqrt(R_DIRECTION.wwww ));
    	R_DIRECTION.w=temp.w;
}
{
    float4 temp = (float4)(R_DIRECTION.xyzz * R_DIRECTION.wwww );
    	R_DIRECTION.x=temp.x;
    	R_DIRECTION.y=temp.y;
    	R_DIRECTION.z=temp.z;
}
{
    float4 temp = (float4)(dot(R_DIRECTION.xyz, r10.xyz));
    	r10.w=temp.w;
}
	R_FADE = max(r10.wwww , -r10.wwww );
...
{
    float4 temp = (float4)(c[4].wwww  + -R_FADE.zzzz );
    	r10.x=temp.x;
}
{
    float4 temp = (float4)(r10.xxxx * r10.xxxx );
    	r10.x=temp.x;
}
{
    float4 temp = (float4)(r10.xxxx * r10.xxxx );
...
{
    float4 temp = (float4)(c[4].wwww  + -r10.xxxx );
    	r10.x=temp.x;
}
{
    float4 temp = (float4)(-c[207].wwww );
    	r11.x=temp.x;
}
{
    float4 temp = (float4)(c[4].wwww  + r11.xxxx );
    	r10.y=temp.y;
}
{
    float4 temp = (float4)((c[207].wwww * r10.xxxx ) + r10.yyyy );
    	r10.x=temp.x;
}
{
    float4 temp = (float4)(r8.wwww * r10.xxxx );
    	r8.w=temp.w;
}
...
{
    float4 temp = (float4)((r10.xyyy * V_PARTICLE_PRIMARY_TEXCOORD.zwww ) + V_PARTICLE_PRIMARY_TEXCOORD.xyyy );
    	r11.x=temp.x;
    	r11.y=temp.y;
}
{
...
{
    float4 temp = (float4)((r10.xyyy * V_PARTICLE_SECONDARY_TEXCOORD.zwww ) + V_PARTICLE_SECONDARY_TEXCOORD.xyyy );
    	r11.x=temp.x;
    	r11.y=temp.y;
}
{
