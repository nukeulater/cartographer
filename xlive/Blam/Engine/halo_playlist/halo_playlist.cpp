...
#include "kablam/kablam_level_cache.h"
...
static e_halo_playlist_loading_result __cdecl halo_playlist_new(const wchar_t* playlist_file_path, s_halo_playlist_container* playlist_container);
    ...
    while (*file_buffer)
    {
        ...
        switch (this->m_current_reader_mode)
        {
	        case _halo_playlist_reader_seek_mode_new_line:
	        {
	            ...
	            {
	                csmemset(&this->m_section_items[this->m_section_buffer_item_count], 0, sizeof(s_halo_playlist_section_item));
	        ...
	        case _halo_playlist_reader_seek_mode_header_read:
	        {
	            ...
	            else
	            {
	                ...
	                if (char_index >= 31)
	                {
	                    ...
	                    this->log_error(_halo_playlist_error_header_name_invalid, this->m_current_reader_line, this->m_section_header_buffer);
	                    this->m_current_reader_mode = _halo_playlist_reader_seek_mode_seek_to_next_line;
	                }
	                else
	                {
	        ...
	        case _halo_playlist_reader_seek_mode_property_name_read:
	        {
	            ...
	            {
	                uint32 section_index = this->m_section_buffer_item_count;
	                uint32 char_index = this->m_current_reader_char_index;
	                if (char_index >= 31)
	                {
	                    this->m_section_items[section_index].name_buffer[char_index] = 0;
	                    this->log_error(_halo_playlist_error_property_name_invalid, this->m_current_reader_line, this->m_section_items[section_index].name_buffer);
	                    this->m_current_reader_mode = _halo_playlist_reader_seek_mode_seek_to_next_line;
	                }
	                else
	                {
	        ...
	        case _halo_playlist_reader_seek_mode_property_value_read:
	        {
	            ...
	            else
	            {
	                ...
	                if (char_index >= 31)
	                {
	                    ...
	                    this->log_error(_halo_playlist_error_property_value_invalid, this->m_current_reader_line, this->m_section_items[section_index].name_buffer, this->m_section_items[section_index].value_buffer);
	                    this->m_current_reader_mode = _halo_playlist_reader_seek_mode_seek_to_next_line;
	                }
	                else
	                {
...
void c_halo_playlist_reader::evaluate_current_header()
{
    ...
    this->m_section_buffer_item_count = 0;
    ...
    switch (header_type)
    {
	    case _halo_playlist_header_playlist:
	    {
	        ...
	        {
	            this->log_error(_halo_playlist_error_playlist_header_already_defined, this->m_current_reader_line);
	        }
	        break;
	    }
	    case _halo_playlist_header_variant:
	    {
	        if (this->m_playlist->variant_count >= k_halo_playlist_maximum_variant_count)
	        {
	            this->log_error(_halo_playlist_error_max_variants, this->m_current_reader_line);
	    ...
	    {
	        if (this->m_match_count < k_halo_playlist_maximum_match_count)
	        {
	        ...
	        {
	            this->log_error(_halo_playlist_error_max_matches, this->m_current_reader_line);
	            this->m_current_section_type = _halo_playlist_header_none;
	        }
	        break;
	    }
		...
		default:
		{
			this->log_error(_halo_playlist_error_header_name_invalid, this->m_current_reader_line);
		}
    }
...
void c_halo_playlist_reader::process_match_section()
{
    ...
    if (valid)
    {
        ...
        if (!match->maximum_players)
            match->maximum_players = k_maximum_players;
...
{
    s_halo_playlist_section_item* section = &this->m_section_items[this->m_section_buffer_item_count];
    ...
    switch (this->m_current_section_type)
    {
	    case _halo_playlist_header_playlist:
	    {
            this->process_playlist_property(section);
	        break;
	    }
	    case _halo_playlist_header_variant:
	    {
            this->process_variant_property(section);
	        break;
	    }
	    case _halo_playlist_header_match:
	    {
            this->process_match_property(section);
	        break;
	    }
    }
...
void c_halo_playlist_reader::process_playlist_property(s_halo_playlist_section_item* section)
{
    ...
    {
        this->log_error(_halo_playlist_error_property_name_invalid, this->m_current_reader_line, section->name_buffer);
        return;
    }
    ...
    switch (property_type)
    {
	    ...
	    case _halo_playlist_playlist_property_pregame_selection_delay:
	    {
	        int32 duration = halo_playlist_item_collection_get_int_time_value(section->value_buffer);
...
void c_halo_playlist_reader::process_variant_property(s_halo_playlist_section_item* section)
{
    bool section_name_exists = false;
    for (uint32 i = 0; i < this->m_section_buffer_item_count; ++i)
    {
        if (!ustrcmp(section->name_buffer, this->m_section_items[i].name_buffer))
        {
            this->log_error(_halo_playlist_error_property_already_defined, this->m_current_reader_line, section->name_buffer);
    ...
    if (!section_name_exists)
    {
        ...
        {
            ++this->m_section_buffer_item_count;
        }
        else
        {
            this->log_error(_halo_playlist_error_property_name_invalid, this->m_current_reader_line, section->name_buffer);
...
void c_halo_playlist_reader::process_match_property(s_halo_playlist_section_item* section)
{
...
{
    bool invalid = false;
    if (this->m_section_buffer_item_count == 0)
    {
        this->log_error(_halo_playlist_error_variant_name_not_found, this->m_current_header_file_line);
        invalid = true;
    }
    else
    {
        bool variant_name_found = false;
        wchar_t variant_name[k_halo_playlist_maximum_text_length]{};
        ...
        wchar_t base_variant_name[k_halo_playlist_maximum_text_length]{};
        ...
        for (uint32 i = 0; i < this->m_section_buffer_item_count; ++i)
        {
            ...
            switch (variant_property_type)
            {
	            case _halo_playlist_variant_property_name:
	            {
	                ...
	                if (!ustrcmp(file_section->value_buffer, L"") ||
	                    wcsncpy_s(variant_name, NUMBEROF(variant_name), file_section->value_buffer, _TRUNCATE))
	                {
	                    this->log_error(_halo_playlist_error_property_value_invalid, variant_file_line, file_section->name_buffer, file_section->value_buffer);
	                }
	            ...
	            case _halo_playlist_variant_property_base_variant:
	            {
	                ...
	                if (!ustrcmp(file_section->value_buffer, L""))
	                {
	                    this->log_error(_halo_playlist_error_property_value_invalid, base_variant_file_line, file_section->name_buffer, file_section->value_buffer);
	                    valid_variant_setup = game_type_found == false;

	                    if (!valid_variant_setup)
	                        this->log_error(_halo_playlist_error_variant_base_variant_and_game_type_both_set, file_section->file_line);
	                }
	                else
	                {
	                    ...
	                    if (!base_game_variant)
	                        this->log_error(_halo_playlist_error_variant_invalid, base_variant_file_line, file_section->name_buffer);
	            ...
	            case _halo_playlist_variant_property_game_type:
	            {
	                ...
	                if (game_type < _game_variant_description_slayer)
	                    this->log_error(_halo_playlist_error_property_value_invalid, game_type_line, file_section->name_buffer, file_section->value_buffer);
        ...
        {
            this->log_error(_halo_playlist_error_variant_base_or_game_type_not_found, this->m_current_header_file_line);
            invalid = true;
        }

        if (!validate_wchar_characters(variant_name) || uniswcntrl(variant_name))
        {
            this->log_error(_halo_playlist_error_variant_name_invalid, variant_file_line);
            invalid = true;
        }

        if (wcsstr(variant_name, L"|"))
        {
            this->log_error(_halo_playlist_error_variant_name_illegal_character, variant_file_line);
            invalid = true;
        }

        if (!invalid
            && ustrcmp(variant_name, L"")
            ...
            )
        {
            ...
            if (this->m_playlist->variant_count == 0)
            {
                if (base_variant_found)
                    ...
                    game_variant_build_default(&new_variant, game_type);
            }
            else
            {
                ...
                {
                    this->log_error(_halo_playlist_error_duplicate_variant_found, variant_file_line, variant_name);
                    return;
                }
            ...
            {
                this->log_error(_halo_playlist_error_variant_invalid, this->m_current_reader_line, variant_name, base_variant_name);
            }
...
bool c_halo_playlist_reader::process_variant_cartographer_setting(s_halo_playlist_section_item* section_item, s_game_variant* variant)
{
	...
	{
		case _halo_playlist_cartographer_setting_engine_mode:
	...
	if (!property_result)
		this->log_error(_halo_playlist_error_property_value_invalid, section_item->file_line, section_item->name_buffer, section_item->value_buffer);
...
bool c_halo_playlist_reader::property_name_is_valid(wchar_t const* property_name)
{
	bool result =
		halo_playlist_item_collection_get_variant_property_type(property_name) != k_halo_playlist_variant_property_invalid ||
		halo_playlist_item_collection_match_settings_property_get_value(property_name) != k_halo_playlist_match_settings_property_invalid ||
		halo_playlist_item_collection_player_property_get_value(property_name) != k_halo_playlist_player_property_invalid ||
		halo_playlist_item_collection_team_property_get_value(property_name) != k_halo_playlist_team_property_invalid ||
		halo_playlist_item_collection_vehicle_property_get_value(property_name) != k_halo_playlist_vehicle_property_invalid ||
		halo_playlist_item_collection_equipment_property_get_value(property_name) != k_halo_playlist_equipment_property_invalid ||
		halo_playlist_item_collection_slayer_property_get_value(property_name) != k_halo_playlist_slayer_property_invalid ||
		halo_playlist_item_collection_king_property_get_value(property_name) != k_halo_playlist_king_property_invalid ||
		halo_playlist_item_collection_oddball_property_get_value(property_name) != k_halo_playlist_oddball_property_invalid ||
		halo_playlist_item_collection_juggernaut_property_get_value(property_name) != k_halo_playlist_juggernaut_property_invalid ||
		halo_playlist_item_collection_ctf_property_get_value(property_name) != k_halo_playlist_ctf_property_invalid ||
		halo_playlist_item_collection_assault_property_get_value(property_name) != k_halo_playlist_assault_property_invalid ||
		halo_playlist_item_collection_territories_property_get_value(property_name) != k_halo_playlist_territories_property_invalid ||
		halo_playlist_item_collection_cartographer_setting_get_value(property_name) != k_halo_playlist_cartographer_setting_count;
...
{
    this->m_section_items[this->m_section_buffer_item_count].name_buffer[this->m_current_reader_char_index] = L'\0';

    for (uint32 i = this->m_current_reader_char_index -1; i > 0; --i)
    {
        wchar_t c = this->m_section_items[this->m_section_buffer_item_count].name_buffer[i];
        ...
        this->m_section_items[this->m_section_buffer_item_count].name_buffer[i] = L'\0';
    }
...
void c_halo_playlist_reader::log_error(e_halo_playlist_error error_type, uint32 file_line, const wchar_t* property_name, const wchar_t* property_value, const wchar_t* extra)
{
    ...
    if (this->m_playlist->error_log_count == k_halo_playlist_maximum_errors) {
        this->m_playlist->error_log_full = true;
        return;
    }

    s_halo_playlist_error* error = &this->m_playlist->errors[this->m_playlist->error_log_count];
    
	error->error_type = error_type;
    error->file_line = file_line;

	ustrncpy(error->property_name, property_name, NUMBEROF(error->property_name));
	ustrncpy(error->property_value, property_value, NUMBEROF(error->property_value));
	ustrncpy(error->property_extra, extra, NUMBEROF(error->property_extra));
    
	++this->m_playlist->error_log_count;
...
{
    if (this->m_current_reader_mode == _halo_playlist_reader_seek_mode_property_name_read)
    {
        this->trim_property_name();
        this->m_current_reader_char_index = 0;
        this->m_current_reader_mode = _halo_playlist_reader_seek_mode_property_value_read;
    ...
    if (this->m_current_reader_mode == _halo_playlist_reader_seek_mode_property_value_read)
    {
        this->process_current_property();
        this->m_current_reader_mode = _halo_playlist_reader_seek_mode_new_line;
    }
    ...
    if (!this->m_playlist->pregame_delay) 
        this->m_playlist->pregame_delay = k_halo_playlist_default_delay_time;
    
    if (!this->m_playlist->postgame_delay)
        this->m_playlist->postgame_delay = k_halo_playlist_default_delay_time;

    if (!this->m_playlist->pregame_team_selection_delay)
        this->m_playlist->pregame_team_selection_delay = k_halo_playlist_default_delay_time;

    qsort_s(this->m_playlist->variants, this->m_playlist->variant_count, sizeof(s_game_variant),
            c_halo_playlist_reader::game_variant_sort, nullptr);

    for (uint32 i = 0; i < this->m_match_count; ++i)
    {
        s_halo_playlist_match* match = &this->m_matches[i];

        s_game_variant* variant = get_default_game_variant_by_name(match->variant);

        if (!variant)
        {
            variant = (s_game_variant*)bsearch_s(match->variant, this->m_playlist->variants, 
                this->m_playlist->variant_count, sizeof(s_game_variant),
                c_halo_playlist_reader::game_variant_search, nullptr);
        }

        if (!variant)
        {
            this->log_error(_halo_playlist_error_match_variant_not_found, match->variant_line_in_file);
            continue;
        }
        ...
        if (!map_found)
        {
            ...
            map_found = map_manager_get()->get_custom_map_list_ids_by_map_name(match->map, &custom_map_id, 1);
        }
        ...
        match_variant->minimum_players = match->minimum_players;
        match_variant->maximum_players = match->maximum_players;
    }

    qsort_s(this->m_playlist->errors, this->m_playlist->error_log_count, sizeof(s_halo_playlist_error), c_halo_playlist_reader::playlist_error_sort, nullptr);
...
{
    s_halo_playlist_error* left = (s_halo_playlist_error*)lhs;
    s_halo_playlist_error* right = (s_halo_playlist_error*)rhs;

    if (left->file_line != right->file_line)
        return left->file_line - right->file_line;

    if (left->error_type != right->error_type)
        return left->error_type - right->error_type;

    int32 result = ustrncmp(left->property_name, right->property_name, NUMBEROF(left->property_value));
    
	if (result)
        return result;

    result = ustrncmp(left->property_value, right->property_value, NUMBEROF(left->property_value));

    if (result)
        return result;

    return ustrncmp(left->property_extra, right->property_extra, NUMBEROF(left->property_extra));
}

/* private code */ 

e_halo_playlist_loading_result __cdecl halo_playlist_new(const wchar_t* playlist_file_path, s_halo_playlist_container* playlist_container)
{
	...
	reader.m_section_buffer_item_count = 0;
	...
	if (file_handle)
	{
		...
		do
		{
			if (fgetws(buffer, NUMBEROF(buffer), file_handle))
			{
	...
	else
	{
		...
		{
			result = _halo_playlist_loading_result_file_not_found;
		}
		else if (GetLastError() == ERROR_INVALID_DATA)
		{
			result = _halo_playlist_loading_result_invalid_data;
		}
	}
