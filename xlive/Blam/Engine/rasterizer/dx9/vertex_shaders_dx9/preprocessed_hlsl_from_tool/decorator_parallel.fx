...
#define AXIS		 va_billboard_axis
 	...
 	r1 = dot(r1.xyz, r1.xyz);
 {
	...
	r1 = AXIS.yzxw * EYE_FORWARD.zxyw ;
 	r1 = (-EYE_FORWARD.yzxw * AXIS.zxyw ) + r1 ;
 

{
    float4 temp = (float4)(dot(r1.xyz, r1.xyz));
    	r1.w=temp.w;
}
{
    float4 temp = (float4)(rsqrt(r1.wwww ));
	...
	OFFSET = va_billboard_offset * SCALE ;
 	r2 = AXIS ;
 {
    float4 temp = (float4)((r2 * OFFSET.zzzz ) + r0 );
    	r0.x=temp.x;
    	r0.y=temp.y;
    	r0.z=temp.z;
}
{
    float4 temp = (float4)((r1 * OFFSET.yyyy ) + r0 );
    	r0.x=temp.x;
    	r0.y=temp.y;
    	r0.z=temp.z;
}
