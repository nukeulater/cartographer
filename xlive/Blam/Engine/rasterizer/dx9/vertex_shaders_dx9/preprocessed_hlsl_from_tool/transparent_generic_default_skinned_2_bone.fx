...
{
    float4 temp = (float4)(dot(_NORMAL.xyz, _NORMAL.xyz));
    	_NORMAL.w=temp.w;
}
{
    float4 temp = (float4)(rsqrt(_NORMAL.wwww ));
    	_NORMAL.w=temp.w;
}
{
    float4 temp = (float4)(_NORMAL * _NORMAL.wwww );
    	_NORMAL.x=temp.x;
    	_NORMAL.y=temp.y;
    	_NORMAL.z=temp.z;
}
{
    float4 temp = (float4)(dot(_BINORMAL.xyz, _BINORMAL.xyz));
    	_BINORMAL.w=temp.w;
}
{
    float4 temp = (float4)(rsqrt(_BINORMAL.wwww ));
    	_BINORMAL.w=temp.w;
}
{
    float4 temp = (float4)(_BINORMAL * _BINORMAL.wwww );
    	_BINORMAL.x=temp.x;
    	_BINORMAL.y=temp.y;
    	_BINORMAL.z=temp.z;
}
{
    float4 temp = (float4)(dot(_TANGENT.xyz, _TANGENT.xyz));
    	_TANGENT.w=temp.w;
}
{
    float4 temp = (float4)(rsqrt(_TANGENT.wwww ));
    	_TANGENT.w=temp.w;
}
{
    float4 temp = (float4)(_TANGENT * _TANGENT.wwww );
    	_TANGENT.x=temp.x;
    	_TANGENT.y=temp.y;
    	_TANGENT.z=temp.z;
}
...
{
	float temptemp;
	temptemp = dot(r9.xyz, c[18 + 0].xyz);
...
{
	float temptemp;
	temptemp = dot(r9.xyz, c[18 + 1].xyz);
