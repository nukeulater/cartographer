...
{
    float4 temp = (float4)(r10.wwww * oPos.xyzz );
    	oPos.x=temp.x;
    	oPos.y=temp.y;
    	oPos.z=temp.z;
}
{
    float4 temp = (float4)(1.0f );
    	oPos.w=temp.w;
}
{
    float4 temp = (float4)((oPos.xyyy * c[48].xyyy ) + c[48].zwww );
    	oPos.x=temp.x;
    	oPos.y=temp.y;
}

{
    float4 temp = (float4)((oPos.xxxx * 2.0f ) + -1.0f );
    	oPos.x=temp.x;
}
{
    float4 temp = (float4)((oPos.yyyy * -2.0f ) + 1.0f );
    	oPos.y=temp.y;
}


{
    float4 temp = (float4)(max(oPos.zzzz , 0.0f ));
    	oPos.z=temp.z;
}
