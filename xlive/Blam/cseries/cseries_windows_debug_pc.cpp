...
static LPTOP_LEVEL_EXCEPTION_FILTER pfn_SecondaryExceptionFilter = NULL;
static LPTOP_LEVEL_EXCEPTION_FILTER(WINAPI* pfn_SetUnhandledExceptionFilter)(LPTOP_LEVEL_EXCEPTION_FILTER);

/* prototypes */

static LONG WINAPI debug_unhandled_exception_cb(_In_ struct _EXCEPTION_POINTERS* ExceptionInfo);
...
static LPTOP_LEVEL_EXCEPTION_FILTER WINAPI SetUnhandledExceptionFilterHook(LPTOP_LEVEL_EXCEPTION_FILTER pfnFilter);
...
{
	PIMAGE_DOS_HEADER dos_header = (PIMAGE_DOS_HEADER)(pe_module);
	PIMAGE_NT_HEADERS nt_headers = (PIMAGE_NT_HEADERS)((uint8*)(pe_module) + dos_header->e_lfanew);
	return nt_headers;
}
...
static LONG WINAPI debug_unhandled_exception_cb(_In_ struct _EXCEPTION_POINTERS* exception_info)
{
	...
	else
	{
		...
		if (result)
		{
			error(_error_log, "SHFileOperationW failed to delete folder %ws with error %d", reports_path.get_string(), result);
		}
	}
