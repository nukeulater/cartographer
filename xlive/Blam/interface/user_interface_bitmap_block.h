...
{
protected:
	struct s_button_widget_reference* m_tag_block;
	...
	uint8 gap_92[2];
	struct bitmap_data* m_bitmap_data;

public:
	const real_vector2d* get_render_scale();
	struct bitmap_data* get_current_bitmap_data();
	void verify_and_change_sprite(int16 index);
	void assign_new_bitmap_block(struct bitmap_data* block);
	...
	virtual ~c_bitmap_widget(void) = default;
