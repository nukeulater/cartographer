...
const DWORD *const k_weather_shaders_bytecode_ps_2_0[k_dx9_weather_shader_type_count]
{
...
const DWORD *const k_weather_shaders_bytecode_ps_3_0[k_dx9_weather_shader_type_count]
{
...
void __cdecl c_particle_system_lite::initialize(void)
{
	...
	{
		const DWORD *const *const bytecode = rasterizer_globals_get()->d3d9_sm3_supported ? k_weather_shaders_bytecode_ps_3_0 : k_weather_shaders_bytecode_ps_2_0;
		for (uint8 i = 0; i < NUMBEROF(g_d3d9_weather_shaders); ++i)
		{
			...
			if (!SUCCEEDED(hr))
			{
				error(_error_delayed, "render:weather: c_particle_system_lite::initialize: CreatePixelShader failed with hr == 0x%08x", hr);
