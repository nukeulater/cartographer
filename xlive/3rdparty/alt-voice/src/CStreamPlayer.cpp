...
CStreamPlayer::CStreamPlayer(long long identifier) : _identifier(identifier)
{
...
{
	return isPlaying 
		&& std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - lastSourcePlayTime).count() < 1000;
}
...
bool CStreamPlayer::Update()
{
	...
	if (!isPlaying)
	{
		...
		if (timeFromFirstBuffer > soundOutput->bufferingTime)
		{
			alSourcePlay(source);
			sourceUsedOnce = true;
			lastSourcePlayTime = std::chrono::system_clock::now();
			if (alGetError() != AL_NO_ERROR)
			{
				DropSource();
				return false;
...
long long CStreamPlayer::GetPlayerIndentifier()
{
	return _identifier;
}
