...
CSoundOutput::CSoundOutput(char* deviceName, int sampleRate, int sourcesCount) : _sampleRate(sampleRate)
{
	...
	alcGetIntegerv(device, ALC_MONO_SOURCES, sizeof(ALCint), &maxMonoSources);
...
IStreamPlayer* CSoundOutput::CreateStreamPlayer(long long identifier)
{
	try
	{
		CStreamPlayer* nextStreamPlayer = new CStreamPlayer(identifier);
...
IStreamPlayer* CSoundOutput::GetStreamPlayer(long long identifier)
{
	for (auto streamPlayer : _streamPlayers)
	{
		if (streamPlayer->GetPlayerIndentifier() == identifier)
			return streamPlayer;
	}
	return nullptr;
}

void CSoundOutput::GetPlayerIdentifiers(int* identifierCount, long long* identifiers)
{
	*identifierCount = _streamPlayers.size();
	if (_streamPlayers.size() > 0)
	{
		int i = 0;
		for (auto streamPlayer : _streamPlayers)
		{
			identifiers[i] = streamPlayer->GetPlayerIndentifier();
			i++;
		}
	}
}
...
void CSoundOutput::UpdateAllStreamPlayers()
{
	for (auto streamPlayer : _streamPlayers)
		streamPlayer->Update();
}
